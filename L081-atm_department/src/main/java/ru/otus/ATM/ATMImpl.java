...
public class ATMImpl implements ATM {
    ...
    public String toString() {
        return "Банкомат (" + this.atmId +") {" + "Баланс=" + String.format("%,d", getBalance()) + "}";
    }

    private void saveInitialState() {
        this.initialState = new Memento(this.cassette);
    }

    @Override
    public void loadInitialState() {
        this.cassette = this.initialState.undo();
    }

    private class Memento {
        private Map<Integer, Integer> cassette;

        private Memento(Map<Integer, Integer> cassette) {
            this.cassette = new TreeMap<>(Collections.reverseOrder());
            this.cassette.putAll(cassette);
        }

        private Map<Integer, Integer> undo() {
